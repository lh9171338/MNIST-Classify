...
class TL(nn.Module):
    ...
    """
    def __init__(self, dim, num_heads, mlp_ratio=4, sr_ratio=1, qkv_bias=False, width=None, height=None, pos_encoding=False):
        super().__init__()
        self.norm1 = nn.GroupNorm(1, dim)
        self.norm2 = nn.GroupNorm(1, dim)

        self.attn = MHSA(dim, num_heads, sr_ratio, qkv_bias, width, height, pos_encoding)
...
class Transformer(nn.Module):
    def __init__(self, in_dim, num_classes, depth, embed_dim, num_heads, mlp_ratio, sr_ratio, qkv_bias, width, height, pos_encoding):
        super().__init__()
